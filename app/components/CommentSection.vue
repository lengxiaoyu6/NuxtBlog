    ...
    <section class="mt-16 space-y-12">
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                ...
                <h2
                    class="text-lg sm:text-2xl font-black text-slate-900 dark:text-white font-serif truncate whitespace-nowrap">
                    评论交流
                    <span class="text-slate-300 dark:text-slate-700 ml-1 sm:ml-2">({{ visibleComments.length }})</span>
                </h2>
            </div>
        ...
        <div v-if="isFormOpen" class="fixed inset-0 z-[70] flex items-center justify-center p-4 pointer-events-none">
            <div
                ...
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    ...
                    <div
                        v-if="captchaEnabled"
                        class="rounded-2xl border border-brand-100 bg-brand-50/70 px-4 py-3 text-xs text-brand-700 dark:border-brand-900/40 dark:bg-brand-950/30 dark:text-brand-200">
...
const captchaHint = computed(() =>
    isGuestbookPage.value ? '留言或回复前，请先完成人机校验。' : '发表评论或回复前，请先完成人机校验。',
);
